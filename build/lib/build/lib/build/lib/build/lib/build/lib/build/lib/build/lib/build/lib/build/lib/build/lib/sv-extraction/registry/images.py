...
class ImagesRegistry():
    ...
    def insert_row(self, config: ImageDataConfig, verbose: bool=False, force=False):   
        #TODO implement row existence check and force
        #TODO use relative echogram_images_dir path when inserting
        return insert_image_ds(self.conn, config, verbose)        #TODO FINISH FUNCTION
...
def insert_image_ds(
    ...
    if row is not None:     # if insert was successful (i.e. new line), the id was returned
        img_ds_id = row[0]  
        if verbose: print(f"Successfully added new image dataset with id {img_ds_id}")
    else:                   # if line already existed, we need to select the id
        img_ds_id =  get_id_from_config(conn, config)
        if verbose: print(f"Image dataset already exists in registry with id {img_ds_id}")
